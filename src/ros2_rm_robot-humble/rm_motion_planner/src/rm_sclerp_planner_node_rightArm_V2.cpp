 ...
 * @updates 2025/9/26，删除定时器机制，改为话题回调机制
 * @updates 2025/9/28，添加触发左臂动作客户端
 * @note 功能测试版本V2, 支持定时器或动作两种设计模式切换 
 * @paramaters: 基于示教采集的数据硬编码 + 订阅感知模块话题数据
 */
    ...
    */
    ...
    void box_centerPoint_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
        ...
        // 位置信息提取(与存储)
      {
        ...
        // 打印接收到的位姿信息
        RCLCPP_INFO(this->get_logger(), "%s 收到盒子中心点位姿 - 位置: [%f, %f, %f], 方向: [%f, %f, %f, %f]", DATA_ICON.c_str(),
    ...
    void tempForShow()
    {
        // 初始化电吸盘(若注释，表示在构造函数中初始化) 
        // RCLCPP_INFO(this->get_logger(), "%s 开始初始化吸盘机械臂",WORKING_ICON.c_str());
        // std::string jodell_initialize_cmd = "ros2 service call /right_arm/initialize_gripper rm_ros_interfaces/srv/InitializeGripper \""
        // "{ip: '192.168.1.19', port: 8080, register_address: 1000, comm_port: 1, device_address: 9}\"";
        // call_service_sync(jodell_initialize_cmd); 
        // RCLCPP_INFO(this->get_logger(), "%s 吸盘初始化完成",COMPLETE_ICON.c_str());
        ...
        on_right_arm_delivery_complete();       
  ...
  // #region TriggerArm 动作触发函数     
    ...
    void trigger_left_arm_task(const std::string& task_description) 
    {
        ...
        send_goal_options.result_callback = 
            std::bind(&RmSclerpPlannerNode1::trigger_result_callback, this, _1);
...
// #endregion   
    ...
    void call_service_sync(const std::string& cmd) 
    {
        ...
        if (result == 0) {
            RCLCPP_INFO(this->get_logger(), "$s 吸盘服务调用成功", SUCCESS_ICON.c_str());
        } else {
    ...
    void initialize_planner() 
    {
        ...
        std::string urdf_path;
        this->get_parameter("urdf_path", urdf_path);
        RCLCPP_INFO(this->get_logger(), "ℹ️ 尝试使用URDF路径: %s", urdf_path.c_str());
        ...
        if (getLinkNamesFromURDF(urdf_path, actual_base_link, actual_tip_link)) {
            ...
            RCLCPP_WARN(this->get_logger(), "⚠️ 无法从 URDF 文件中获取链接名称，使用默认值: base='%s', tip='%s'", 
                        base_link.c_str(), tip_link.c_str());
        }

        // 创建ScLERP运动规划器实例
        try {
            RCLCPP_INFO(this->get_logger(), "ℹ️ 使用指定的URDF文件: %s", urdf_path.c_str());
                        ...
                        trajectory_msgs::msg::JointTrajectory &joint_trajectory )
    {
        ...
        RCLCPP_INFO(this->get_logger(), "%s 开始执行sclerp_planner函数", SIGNAL_ICON.c_str());

        // 检查 planner_interface_ 是否已正确初始化
        if (!planner_interface_) {
            RCLCPP_ERROR(this->get_logger(), "%s sclerp_planner：规划器接口未初始化", ERROR_ICON.c_str());
            return;
        }
        ...
        try {
            ...
            RCLCPP_INFO(this->get_logger(), "%s 规划成功，轨迹点数量: %ld", SUCCESS_ICON.c_str(), joint_trajectory.points.size());
        ...
        if (success && !joint_trajectory.points.empty()) 
        {
            ...
            RCLCPP_ERROR(this->get_logger(), "%s 规划失败!", ERROR_ICON.c_str());
        
            // 打印规划失败时的详细信息
            RCLCPP_ERROR(this->get_logger(), "%s 目标位姿矩阵 matrix_base2tcp_target:",INFO_ICON.c_str());
            for (int i = 0; i < 4; i++) {
        ...
        RCLCPP_INFO(this->get_logger(), "%s sclerp_planner函数执行完成， %s 总耗时:  %.6f 秒", COMPLETE_ICON.c_str(), TIMER_ICON.c_str(), duration.count());
    }
...
{
    RCLCPP_INFO(this->get_logger(), "%s 正在初始化机械臂...", INITIALIZE_ICON.c_str()); 
    try {
        // 初始化机械臂API
        ...
        RCLCPP_ERROR(this->get_logger(), "%s 机械臂初始化过程中发生异常: %s", ERROR_ICON.c_str(), e.what());
    }
