...
// #region TriggerArm 动作服务端相关函数
        ...
        const std::shared_ptr<TriggerArmHandle> goal_handle)
    {
        ...
        // 发布反馈 - 开始执行
        feedback->status = "开始执行右臂抓取任务";
        goal_handle->publish_feedback(feedback);
        
        // 执行右臂任务（吸取物体、扫码、放置等）
        bool success = execute_right_arm_task();
        ...
        if (success) {
            ...
            result->success = false;
            result->message = "右臂任务执行失败";
            goal_handle->abort(result);
            RCLCPP_ERROR(this->get_logger(), "右臂任务执行失败");
        }
    }
...
// #endregion   
  ...
  // #region TriggerArm 动作客户端相关函数     
   /* 
    ...
    {
        (void)goal_handle;
        RCLCPP_INFO(this->get_logger(), "右臂任务反馈: %s", feedback->status.c_str());
    }
...
// #endregion   
                        ...
                        trajectory_msgs::msg::JointTrajectory &joint_trajectory )
    {
        ...
        try {
            ...
            RCLCPP_INFO(this->get_logger(), "%s 规划器返回结果: %s", SIGNAL_ICON.c_str(), success ? "成功" : "失败");
