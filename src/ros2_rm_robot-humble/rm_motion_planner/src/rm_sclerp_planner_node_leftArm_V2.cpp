...
public:
    ...
    /************************ 参数申明与获取 ************************/
        // 声明参数
        this->declare_parameter<std::string>("start_joint_positions",  "-1.413, -1.608, -0.528, -1.828, -1.491, 2.132, -0.265");
        ...
        // 初始化机械臂
        initialize_rmRobot();
        // 初始化电吸盘（不能放在构造函数中，外部动作客户端多次调用会导致服务响应失败）
    ...
    /***************** 通信：话题，服务，动作 ******************/        
        ...
        RCLCPP_INFO(this->get_logger(), "rm_sclerp_planner_node_leftArm 节点已经启动.");
...
// #region TriggerArm 动作回调函数
        ...
        const std::shared_ptr<TriggerArmHandle> goal_handle)
    {
        ...
        auto task = std::bind(&RmSclerpPlannerNode1::execute_trigger_left_arm, this, _1);
