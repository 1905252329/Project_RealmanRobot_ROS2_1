...
void RmArm::Arm_Get_Force_Data_Callback(const std_msgs::msg::Empty::SharedPtr msg)
{
    ...
    // 删除此处的#include "rm_ros_interfaces/srv/modbus_control.hpp"
...
{
    (void)request; // 显式标记未使用参数
    
    // 调用API函数写入外设寄存器
    // int result = Rm_Api.rm_peripheral_write(robot_handle, request->port, request->address, request->device, request->data);
    // 由于rm_peripheral_write函数不存在，暂时注释掉此行
    int result = -1; // 设置默认错误值
    
    if (result == 0) {
        response->success = true;
        response->message = "Modbus write successful";
    } else {
        response->success = false;
        response->message = "Modbus write failed with error code: " + std::to_string(result);
    }
}
...
{
    (void)request;
    RCLCPP_INFO(this->get_logger(), "Received GetDH service request");
    
    // 获取DH参数
    rm_dh_t dh_data;
    int res = rm_get_DH_data(robot_handle, &dh_data);
    ...
    if (res == 0) {
        ...
        // 填充DH参数到响应中 (最多7个关节)
        for (int i = 0; i < 7; i++) {
            response->a[i] = dh_data.a[i];
            response->d[i] = dh_data.d[i];
            response->alpha[i] = dh_data.alpha[i];
            response->offset[i] = dh_data.offset[i];
        }
        
        RCLCPP_INFO(this->get_logger(), "Successfully retrieved DH parameters");
    } else {
        // 获取DH参数失败
        response->success = false;
        RCLCPP_ERROR(this->get_logger(), "Failed to retrieve DH parameters, error code: %d", res);
    }
}
...
/*******************************************************************************end*****************************************************************/
    ...
    // 创建 Modbus 控制服务
    modbus_control_service_ = this->create_service<rm_ros_interfaces::srv::ModbusControl>(
        "modbus_control", std::bind(&RmArm::modbusControlCallback, this, std::placeholders::_1, std::placeholders::_2));
        
    // 创建 GetDH 服务
    get_dh_service_ = this->create_service<rm_ros_interfaces::srv::GetDH>(
        "rm_driver/get_dh", std::bind(&RmArm::getDHCallback, this, std::placeholders::_1, std::placeholders::_2));
...
int main(int argc, char *argv[])
{
    ...
    // 创建UDP发布节点
    auto udp_node = std::make_shared<UdpPublisherNode>();
    // 运行节点
