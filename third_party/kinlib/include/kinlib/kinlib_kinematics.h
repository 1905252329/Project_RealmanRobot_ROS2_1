...
class KinematicsSolver
{
  public:
    ...
    */
    ...
    /*!
      \brief    Get the gst0_ (tool transformation) matrix

      \details  Returns the reference configuration (gst0_) of the manipulator,
                which represents the transformation from the last joint to the tool center point

      \return   Reference configuration of the manipulator
    */
    Eigen::Matrix4d getGst0() { return manipulator_.getReferenceConfiguration(); }
  ...
  private:
    ...
    */
    ...
    /*!
      \brief    link meshes for collision check
    */
    std::vector<std::shared_ptr<CollisionUtils::ObstacleBase>> link_meshes_;

    /*!
      \brief    link meshes offset for collision check
    */
